...
import cn.fred.tv.databinding.ActivityPlayerBinding
import cn.fred.tv.dialog.ChannelListDialog
...
class PlayerActivity : BaseActivity() {
    ...
    private var tvList = mutableListOf<Channel>()
    private var exoPlayer: ExoPlayer? = null
    private var dialog: ChannelListDialog? = null
    private var exitFlag = false
    private var lastTV: Channel? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        tvJson?.let { json ->
            val tv = Gson().fromJson(json, Channel::class.java)
            tv?.let {
                lastTV = tv
...
//                binding.exoPlay.setShutterBackgroundColor(Color.TRANSPARENT)
        ...
        binding.exoPlay.setErrorMessageProvider(object : ErrorMessageProvider<PlaybackException> {
            override fun getErrorMessage(throwable: PlaybackException): Pair<Int, String> {
                Log.e(
                    TAG,
                    "getErrorMessage: code:${throwable.errorCode}  name:${throwable.errorCodeName}  retryCount:$retryCount"
                )
